...
def trade_resource(inventory):
    ...
    if response.lower() in acceptable_yes:
      if inventory[trading_resource] < trading_quantity:
        input(f'You attempted to trade {trading_quantity} {trading_resource} for {offered_quantity} {offered_resource}. Unfortunately, you do not have enough {trading_resource}!')
